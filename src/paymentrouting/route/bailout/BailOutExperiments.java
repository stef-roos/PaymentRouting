...
//		double p = Double.parseDouble(args[1]); 
	...
	public static void lightningDelayLocal() {
		Config.overwrite("SKIP_EXISTING_DATA_FOLDERS", ""+false);
		Config.overwrite("MAIN_DATA_FOLDER", "./data/bailout/");
		String file  = "lightning/lngraph_2020_03_01__04_00.graph";
		...
		m[0] = new RoutePaymentBailout(new LND(new HopDistance()),1,0.1, "data/bailout/records-Delay-Test.txt",reacts[0],
									bfees[0], facs[0], afees[0], thres[0], wait); 
	...
	public static void summarize() {
		...
		Metric[] m = new Metric[reacts.length*bfees.length*afees.length+reacts.length];
		for (int i = 0; i < reacts.length; i++) {
		   m[i] =  new RoutePaymentBailout(new LND(new HopDistance()),1,0.1, "data/bailout/records-Delay-"+i+".txt",reacts[i],
						BailoutFee.NEVER, 0, AcceptFee.ALWAYS, 0, wait); 
		} 
		for (int i = 0; i < reacts.length; i++) {
			for (int j = 0; j < bfees.length; j++) {
				for (int k = 0; k < afees.length; k++) {
					try {
					m[reacts.length+k*bfees.length*reacts.length+j*reacts.length+i] = 
							new RoutePaymentBailout(new LND(new HopDistance()),1,0.1, "data/bailout/records-Delay-"+i+"-"+j+"-"+k+".txt",reacts[i],
									bfees[j], facs[j], afees[k], thres[k], wait); 
					} catch (Exception e) {
						e.printStackTrace();
					}
