...
public class EdgeListGraphReader extends GraphReader {
	...
	public EdgeListGraphReader(String separator) {
		this("EDGE_LIST", separator, false);
	}
	
	public EdgeListGraphReader(String separator, boolean makeBidirectional) {
		this("EDGE_LIST", separator, false);
	}

	protected EdgeListGraphReader(String key, String separator, 
			boolean makeBidirectional) {
		super(key);
	...
	public Graph read(String filename) {
		...
		try {
			...
			while ((line = fr.readLine()) != null) {
				...
				if (this.mBi) {
					edges.add(map.get(temp[1]), map.get(temp[2]));
				}
			}
			edges.fill();
