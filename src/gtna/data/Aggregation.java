...
public class Aggregation {
	...
	public static boolean aggregateFrom(String[] from, String to, double z,
			int runs) {
	...
	private static boolean aggregateSingle(String[] from, String to,
		...
		for (String key : keys) {
			if (key.contains("RUNTIME")) continue;
			double[] values = new double[lists.length];
			for (int i = 0; i < lists.length; i++) {
				if (lists[i] == null){
					System.out.println("NULL " + key + " " + i + " " + from[0]);
				}
	...
	public static Vector<String> sumFolder(String folder, double z){
		int num = 0;
		while ((new File(folder+"/"+num)).exists()){
			num++;
		}
		
		//iterate over metrics
		String[] mets = (new File(folder+"/0")).list();
		System.out.println("folder " + folder);
		Vector<String> metNames = new Vector<String>();
		for (int i = 0; i < mets.length; i++){
			try{ 
			if ((new File(folder+"/0/"+mets[i])).isDirectory()){
				//do singles
				metNames.add(mets[i]);
				String[] from = new String[num];
				for (int j = 0; j < num; j++){
					from[j] = folder + "/" + j + "/"+mets[i]+"/_singles.txt";
					
				}
				aggregateFrom(from, folder+"/"+mets[i]+"/_singles.txt", z, num);
				
				//do dists 
				String[] files = (new File(folder + "/" + 0 + "/"+mets[i])).list();
				for (int k = 0; k < files.length; k++){
					if (files[k].equals("_singles.txt")) continue;
					if (files[k].contains("graph")) continue;
					...
					double[][] values = extractValues(data, x, cdf);
					String filename = folder + "/" + mets[i] +"/"+files[k];
					Aggregation.writeAggregation(x, values, z, filename);
				}
			}
